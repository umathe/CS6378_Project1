...
ServerSocket ssoc = null;
		...
		while(true) {
			try {
				...
				for(int i = 0; i < totalNodes; i++){
					try {
						Thread.sleep(this.nodeNumber*1000);
						serverCommunicate();
	...
	public static void main(String[] args) {
		...
		//NodeSetup4 n1 = new NodeSetup4();
		n1.nodeNumber = Integer.parseInt(info_nodes[4][0]);
		nodePortNumber = Integer.parseInt(info_nodes[4][2]);
		nodeHostName = info_nodes[4][1];
		nodeNeighbors = info_nodes[4][3];
		...
		Thread t2 = new Thread(new Runnable() {
			public void run() {
				...
				//delayInterval.schedule(taskServerCommDelay, 20000);
			}
		});
		t1.start();
		t2.start();
